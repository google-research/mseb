...
def _get_environment(sub_task: str) -> str:
  ...
  Examples:
    'span_reasoning_in_lang:clean' -> 'clean'
...
class SVQSpanInLangReasoning(reasoning.ReasoningTask):
  ...
  def embeddings_dir(self) -> str:
    ...
    name = f'svq_{self.locale}_span_reasoning_in_lang'
  ...
  def _task_data(self, task_data_key: str, dtype: dict[str, Any] | None = None):
    ...
    df = df[df['reasonings/span_in_lang']]
    if self.size is not None:
      mask = df['span_context_id_in_lang'].map(
  ...
  def multimodal_inputs(self) -> Iterable[types.SoundWithTitleAndContext]:
    default_context_key = 'span_context_text_in_lang'
    ...
    df = self._task_data(
        ...
        dtype={
            'locale': str,
            'utt_id': str,
            'span_context_title_in_lang': str,
            context_key: str,
        },
    )
    for example in df.to_dict('records'):
      ...
      yield types.SoundWithTitleAndContext(
          ...
          title_text=example['span_context_title_in_lang'],
  ...
  def examples(
    ...
    df = self._task_data(
        ...
        dtype={
            ...
            'utt_id': str,
            'span_in_lang': str,
            'spans_in_lang': Sequence[str],
        },
    )
    for example in df.to_dict('records'):
      yield reasoning_evaluator.ReasoningSpans(
          sound_id=example['utt_id'],
          reference_answer=example['span_in_lang'],
          texts=example['spans_in_lang'],
      )
  ...
  def span_lists(self) -> Iterable[Sequence[types.Text]]:
    ...
    for example in df.to_dict('records'):
      yield [
          ...
          for span in example['spans_in_lang']
      ]
...
for _locale, (_suffix, _eval_lang) in _SVQ_LOCALES.items():
  _cls = _make_task_class(  # pylint: disable=invalid-name
      base_cls=SVQSpanInLangReasoning,
      locale=_locale,
      size=None,
      suffix=_suffix,
