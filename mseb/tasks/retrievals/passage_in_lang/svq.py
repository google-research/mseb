...
def _get_environment(sub_task: str) -> str:
  ...
  Examples:
    'passage_retrieval_in_lang:clean' -> 'clean'
...
class SVQPassageInLangRetrieval(retrieval.RetrievalTask):
  ...
  def _task_data(self, task_data_key: str, dtype: dict[str, Any] | None = None):
    ...
    df = df[(df['retrievals/passage_in_lang'])]
    if self.size is not None:
      mask = df['passage_id_in_lang'].map(
  ...
  def examples(
    ...
    df = self._task_data(
        ...
        dtype={'locale': str, 'utt_id': str, 'passage_id_in_lang': str},
    )
    for example in df.to_dict('records'):
      yield retrieval_evaluator.RetrievalReferenceId(
          sound_id=example['utt_id'], reference_id=example['passage_id_in_lang']
      )
