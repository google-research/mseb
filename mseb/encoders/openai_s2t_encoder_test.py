    ...
    self._client.audio.transcriptions.create.return_value = (
        TranscriptionVerbose(
            ...
            words=[
                ...
                TranscriptionWord(end=1.440000057220459, start=1.0, word='red'),
                TranscriptionWord(
                ...
                TranscriptionWord(
                    end=2.5199999809265137, start=2.359999895095825, word='blue'
                ),
            ],
        )
    )
...
class OpenAISpeechToTextEncoderTest(absltest.TestCase):
  ...
  def test_encode(self):
    llm_encoder = MockOpenAISpeechToTextEncoder()
    llm_encoder.setup()
    outputs = llm_encoder.encode([self.test_sound])
    ...
    self.assertEqual(outputs[0].timestamps[0, 1], 3.069999933242798)
