...
"""Speech-to-text encoder using the LiteLLM Transciptions API.
"""
...
class LiteLLMSpeechToTextEncoder(encoder.MultiModalEncoder):
  ...
  def _check_input_types(self, batch: Sequence[types.MultiModalObject]) -> None:
    if not all(isinstance(x, types.Sound) for x in batch):
      raise ValueError(
          'LiteLLMSpeechToTextEncoder only supports a batch of all Sound'
          ' inputs.'
      )
