...
""",
  ...
  def _load_data(self) -> pd.DataFrame:
    ...
    # Convert ebird_code_multilabel to list of text labels
    if (
        "ebird_code_multilabel" in df.columns
        and self._ebird_code_names is not None
    ):
      df["ebird_code_multilabel"] = df["ebird_code_multilabel"].apply(
          lambda indices: [self._ebird_code_names[i] for i in indices]
  ...
  def get_sound(self, record: pd.Series) -> types.Sound:
    ...
    text_labels = record.get("ebird_code_multilabel")
    text_label = ", ".join(text_labels) if text_labels else None
